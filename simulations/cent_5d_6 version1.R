...
make_data <- function(n = 1e2, 
                      ...
                      M1_success = "plogis(-1 + 0.45 * C$C1 + 0.125 * A + 0.5 * C$C1 * A)",
                      M2_success = "plogis(-1 + 0.2 * C$C2 - 0.2 * A + 0.25 * C$C2 * A)",
                      M1M2_threshold = 3,
                      Y_success = "-1 + C$C1 - C$C2 + 0.25 * M1 + 0.02 * M1^2 - 0.25 * M2 - 0.01*M2^2 + 0.25 * A + 0.5 * M1*A - 0.5*M2*A",
                      get_truth = FALSE){
...
if (args[1] == 'run') {
  ...
  for (i in (id+TASKID):(id+TASKID+STEPSIZE-1)) {
    ...
    gcomp_est <- unlist(get_gcomp(full_data))
    
    # g-comp confidence interval(bootstrap)
    M <- 1000
    # empty vectors to hold results
    boot_gcomp_direct <- boot_gcomp_indirectM1 <-boot_gcomp_indirectM2 <- boot_gcomp_total <- rep(NA, M)
    ...
    for(m in 1:M){
      ...
      boot_gcomp_est <- get_gcomp(boot_data)
    ...
    gcomp_cil <- c(apply(cbind(boot_gcomp_total, boot_gcomp_direct, boot_gcomp_indirectM1, boot_gcomp_indirectM2), 2, quantile, p = 0.025), 0)
    gcomp_ciu <- c(apply(cbind(boot_gcomp_total, boot_gcomp_direct, boot_gcomp_indirectM1, boot_gcomp_indirectM2), 2, quantile, p = 0.975), 0)

    # get truth
    set.seed(1234)
    truth <- c(unlist(make_data(n = 1e6, get_truth = TRUE)))
    ...
    gcomp_cover <- rep(NA, 5)
    for(j in seq_len(5)){
      gcomp_cover[j] <- in_ci(truth[j], c(gcomp_cil[j], gcomp_ciu[j]))
    }
    ...
    names(out) <- c("n", "seed", 
                    ...
                    "total_gcomp_cil", "direct_gcomp_cil", "indirectM1_gcomp_cil", "indirectM2_gcomp_cil", "covarM1M2_gcomp_cil",
                    "total_gcomp_ciu", "direct_gcomp_ciu", "indirectM1_gcomp_ciu", "indirectM2_gcomp_ciu", "covarM1M2_gcomp_ciu",
                    "total_gcomp_cover", "direct_gcomp_cover", "indirectM1_gcomp_cover", "indirectM2_gcomp_cover", "covarM1M2_gcomp_cover",
