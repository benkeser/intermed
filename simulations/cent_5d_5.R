...
make_data <- function(n = 1e2, 
                      ...
                      M1_success = "plogis(-1.4 + 0.45 * C$C1 + 0.125 * A)",
...
if (args[1] == 'run') {
  ...
  for (i in (id+TASKID):(id+TASKID+STEPSIZE-1)) {
    ...
    save(out, file = paste0("~/intermed/out/newfit5_", i, ".RData"))
  }
}
...
if (args[1] == 'merge') {   
  ...
  for(i in 1:nrow(parm)){
    tmp <- tryCatch({
      load(paste0(save_dir, "newfit5_", i, ".RData"))
      out
    }, error = function(e){
      rep(NA, n_out)
    })
  ...
  save(out, file = "~/intermed/all_out_intermed_new5.RData")
  ...
  format_out <- function(out, summary_fn,
    ...
    if(!grepl("get_density", summary_fn)){    
      all_out <- sapply(all_eff, function(x){
        ...
        # 2 means parametric
        colnames(a) <- c("tmle", "os", "tmle2", "os2", "n"); row.names(a) <- NULL; a
      }, simplify = FALSE)
    }else{
      all_out <- sapply(all_eff, function(x){
