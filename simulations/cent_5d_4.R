...
if (args[1] == 'run') {
  ...
  for (i in (id+TASKID):(id+TASKID+STEPSIZE-1)) {
    ...
    save(out, file = paste0("~/intermed/out/newfit4_", i, ".RData"))
  }
}
...
if (args[1] == 'merge') { 
  ...
  for(i in 1:nrow(parm)){
    tmp <- tryCatch({
      load(paste0(save_dir, "newfit4_", i, ".RData"))
      out
    }, error = function(e){
      rep(NA, n_out)
    })
  ...
  save(out, file = "~/intermed/all_out_intermed_new4.RData")
